  ...
  for (int i = 0; i < 4; i++) {
    for (int k = 0; k < 4; k++) {
      glPushMatrix();
		...
		glPushMatrix();
			double gx, gy, gz,gxx=cos(ppp / 3.0 * TO_RADIANS)*4,gyy=0,gzz=cos(ppp / 3.0 * TO_RADIANS)*4;
			glTranslated(-cos(ppp / 3.0 * TO_RADIANS)*4, 0, -cos(ppp / 3.0 * TO_RADIANS) * 4);
			for (int j = 0; j < 3600; j++) {
				...
				glVertex3d(gx - 3, gy, gz);
				gxx = cos((j+ppp+1) / 3.0 * TO_RADIANS) *4, gyy = (j + 1.0) / 15.0, 0;
				gzz=cos((j + ppp+1) / 3.0 * TO_RADIANS) * 4;
